...
Javascript 的代码不会被浏览器直接执行，而是通过 JavaScript 引擎重新编译代码使其运行的更快。这种引擎就是 Just In Time，JIT

## JS 相关的优化

在每一帧开始的时候，尽早的去执行 JavaSctipt

要想保证浏览器的流畅性，浏览器需要在 1s 之内渲染出 60 帧，也就是说大约需要 16ms 的时间去渲染一帧，而且浏览器还有别的事情需要做，这时的时间花费大约就被压缩到 10ms 左右

而 Js 代码的执行大约最多有 3 或 4 ms 去执行，因为还需要去做其他的工作如 style calculations，layer management，compositing 等任务

### 创建动画的好工具-RequestAnimaionFrame

假如我们考虑一下这种情况：

![situation-1](/assets/img/15127158597633.jpg)

浏览器在计算一些关于 style 的工作，这时遇到了 Js 代码。浏览器就需要在处理其他任务前去做关于 Js 的工作，而 Js 可能会造成帧的重新返工，也就意味着可能丢失该帧

RequestAnimaionFrame 的就解决了这一问题，它保证将 JS 尽可能的在前面执行

![situation-2](/assets/img/15127162173224.jpg)


但是在一些老的 web 代码中，使用 `setTimeout` 或者 `setInterval` 来处理动画问题。JQuery 甚至现在还在使用。而这两个方法的问题是，并不会注意渲染管道的绘制顺序

[RequestAnimaionFrame-使用](https://gist.github.com/paulirish/1579671)

### 记录 JS 的运行时间-RunningJS
...
有很多瓶颈的问题阻止着有趣的应用从负载的服务器被移植到 Javascript 客户端。这些问题包括浏览器的稳定性，静态类型，可达性和性能表现。幸运的是，当浏览器供应商快速的改善 JavaScript 引擎速度，后面这些问题快速成为了过去。
...
Web Worker 标准定义是一个为了在 Web 应用上，产生背景脚本的 API。Web Worker 允许你产生一个需要长时间运行用于计算目标任务的脚本，而且这个脚本并不会阻塞 UI 或者其他用于处理用户交互的脚本。
...
结构话的拷贝是好的，单也意味着可能会花费上百毫秒的时间。为了应付性能的瓶颈，可以使用 Transferable Objects.
...
在这种情况下，第一个参数是数据第二个参数是需要被转换的项目列表。第一个参数不一定是 ArrayBuffer，也可像下面是 JSON 对象
